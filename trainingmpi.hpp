// #ifndef TRAININGMPI_HPP
// #define TRAININGMPI_HPP
...
void trainNeuralNetwork(NeuralNetwork& neuralNetwork, const std::vector<std::vector<double>>& trainingData, unsigned int epochs, int rank, int size) {
    ...
    for (unsigned int epoch = 0; epoch < epochs; ++epoch) {
        ...
        for (const auto& data : trainingData) {
            ...
            for (int layerIdx = neuralNetwork.layers_.size() - 1; layerIdx >= 0; --layerIdx) {
                ...
                for (unsigned int neuronIdx = rank; neuronIdx < neurons.size(); neuronIdx += size) {

                    //calcolo della derivata della activation function (sigmoide) per calcolo dell'errore
